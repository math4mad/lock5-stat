...
@app.cell
def _(mo):
    mo.md(r"""# Lizard twitch response to fire ants

Port of `UnitB/ch04/adapt-noadapt-behavior-to-fireants.jl`. F-test for equal
variances then a two-sample equal-variance t-test of `Twitches` between
`Invaded` and `Uninvaded` habitats.""")
    return


@app.cell
def _(lock5, np, pl):
    desc = lock5.Lock5Table(354, "FireAnts", "蜥蜴对适应性的行为的差异性", ["Habitat", "Twitches"])
...
def _(g1, g2, lock5, mo):
    t_res = lock5.equal_var_ttest(g1, g2)
    mo.Html(lock5.gt_ttest(t_res).as_raw_html())
    return t_res
