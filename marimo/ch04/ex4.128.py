...
@app.cell
def _(mo):
    mo.md(r"""# Ex 4.128 — diet intervention and BMI (BMIDiff)

Port of `UnitB/ch04/ex4.128.jl`. Two-sample equal-variance t-test of `BMIDiff`
...
@app.cell
def _(lock5, np, pl):
    desc = lock5.Lock5Table(350, "DietDepression", "Can a Brief Diet Intervention Change BMI?",
                            ["Group", "BMIDiff"])
    df = lock5.load_data(desc.name).select(desc.feature)
    gdf = df.group_by("Group", maintain_order=True).agg(pl.col("BMIDiff"))
