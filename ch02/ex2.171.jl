...
## 2. data processing
desc=Lock5Table(136,"HollywoodMovies","Do Movie Budgets Differ Based on the Genre of the Movie?",["Genre","Budget"])
...
function plot_boxplot()
    ...
    ax=Axis(fig[1,1];title="diffeent genre and budget",xticklabelrotation=0.4)
    Box(fig[1,1];color = (:orange,0.1),strokewidth=0.5)
    ax.xticks=(1:length(cats), [cats...])
    for (idx,df) in enumerate(gdf)
         n=nrow(df)
         boxplot!(ax,fill(idx,n),df.Budget)
    end
    fig
end

fig=plot_boxplot()
#save("./ch02/imgs/hollywood-movie-genre-budget.png",fig)
