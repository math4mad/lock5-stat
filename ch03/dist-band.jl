...
function plot_level_band(ax::Axis,params::PvalueParams)
    ...
    as2=Vector(range(cquantile(params.dist,params.level/2),h,100))
    ys_low1=ys_low2 =fill(0,100)
    ys_high1 = pdf.(params.dist,as1)
    ys_high2 = pdf.(params.dist,as2)
   return  (band!(ax, as1,ys_low1, ys_high1,color=(:green,0.5)),
    band!(ax, as2,ys_low2, ys_high2,color=(:green,0.5)))
end
...
#band!(ax, as1,ys_low1, ys_high1,color=(:green,0.5))
#band!(ax, as2,ys_low2, ys_high2,color=(:green,0.5))
